...
- Input is a multiple sequence (time-series) data $(x_1^{(i)}, x_2^{(i)}, \dots, x_{n_i}^{(i)}) \in \mathbb{R}^d~(i=1,\dots, I)$, where $i$ is a sequence index, $n_i$ is the number of samples for \(i\)th sequence (sequence length), and $d>0$ is the dimension of data (assuming \(x_{t+1}^{(i)} = F(x_{t}^{(i)})\)). 
...
* <a href="https://github.com/yusuke-imoto-lab/eps-attracting-basin/blob/main/example_simple.ipynb" target="_blank" rel="noopener noreferrer">**example_Dorphin.ipynb**</a>: An application exmaple pf ensemble weather forecast data for tropical cyclone (typhoon) ``Drophin'', generated by the Meso-scale Ensemble Prediction System (MEPS), provided by the Japan Meteorological Agency (JSA).
